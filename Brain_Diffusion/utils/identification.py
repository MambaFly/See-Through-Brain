...
def load_feat_org(imgid, subject, dir, usefeat):
    featdir = f'../../Brain-Identification/{subject}/cross-subj/{dir}/'
    feat = np.load(f'{featdir}/{imgid:05}_org_{usefeat}.npy').flatten().squeeze()
    return feat

def load_feat_gen(imgid, subject, dir, usefeat):
    featdir = f'../../Brain-Identification/{subject}/cross-subj/{dir}'
    nrep = 5
...
def main():
    ...
    img_start = 800
    img_end = 1000
    nimage = img_end - img_start
    
    # Load all images
    print("Now Loading all images......")
    feat_orgs = []
    feat_gens = []
    for imgid in tqdm(range(img_start, img_end)):
        feat_org = load_feat_org(imgid, subject, dir, usefeat)
        feat_orgs.append(feat_org)

        feat_gen = load_feat_gen(imgid, subject, dir, usefeat)
    ...
    print(f'{subject}_{dir}_{usefeat}:\t ACC = {np.mean(acc_all):.03} ')
