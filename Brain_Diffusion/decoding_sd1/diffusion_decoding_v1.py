...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="source subject name: subj01 or subj02  or subj05  or subj07 for full-data subjects ",
    )
    # parser.add_argument(
    #     "--method",
    #     required=True,
    ...
    outdir = f'../../Brain-Decoded/cross-subj/{subject}/image-{method}-{subject}-to-{source}/'
    ...
    for imgidx in range(imgids[0],imgids[1]):
        ...
        if method in ['init','text']:
            roi_latent = 'early'
            init_latent = np.load(f'../data/decoded/{subject}/{subject}_to_{source}_{roi_latent}_brain_embs_init_latent.npy')
            imgarr = torch.Tensor(init_latent[imgidx-imgids[0],:].reshape(4,40,40)).unsqueeze(0).to('cuda')
